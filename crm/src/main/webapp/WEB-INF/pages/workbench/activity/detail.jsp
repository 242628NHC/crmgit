...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveCreateActivityRemarkBtn").click(function () {
			...
			var activityId='${activity.id}';//当本行js代码在浏览器执行时，activity.id已经变成32位字符串了。
			//表单验证
			if (noteContent==""){
				alert("备注内容不能为空");
				return;
		...
		//给所有的"删除"图标添加动态绑定单击事件
		$("#remarkDivList").on("click","a[name='deleteA']",function () {
			 //收集参数this表示当前DOM对象，加上$()就是jquery对象
		...
		//给所有的"修改"图标添加动态绑定单击事件
		$("#remarkDivList").on("click","a[name='editA']",function () {
			//获取备注的id和noteContent
			var id=$(this).attr("remarkId");
			var noteContent=$("#div_"+id+" h5").text();
			//备注的id和noteContent写到修改备注的模态窗口中
			$("#edit-id").val(id);
